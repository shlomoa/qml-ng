...
2. Recursively parse every `.qml` and `.ui.qml` file under `examples/FigmaVariants` and `examples/WebinarDemo`.
3. Treat `examples/FigmaVariants/FigmaVariants.qmlproject` and `examples/WebinarDemo/WebinarDemo.qmlproject` as the project anchors so the validator always covers the declared app entrypoints.
4. Follow parser-resolved child component source paths from the entrypoints so the validator proves the parser can map local component names such as `MainApp` to their backing `.ui.qml` files.
